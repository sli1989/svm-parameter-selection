function [BestMSE,BestParams,ABCOpts] = ABCSVMcgForRegress(train_label,train_data,ABCOpts)
...
if nargin == 2
    ABCOpts = struct( 'ColonySize',  20, ...   % Number of Employed Bees+ Number of Onlooker Bees
        ...
        'lb',  [2^-10,2^-10],... % Lower bound of the parameters to be optimized
        'ub',  [2^10,2^10],... %Upper bound of the parameters to be optimized
        'v',3,...
...
for r=1:ABCOpts.RunTime
    ...
    for nind = 1:FoodNumber
        cmd = ['-v ',num2str(ABCOpts.v),' -c ',num2str(Foods(nind,1)),' -g ',num2str(Foods(nind,2)),' -s 3 -p 0.01'];
    ...
    while ((iter <= ABCOpts.MaxCycles)),
        ...
        for i=1:(FoodNumber)
            ...
            cmd = ['-v ',num2str(ABCOpts.v),' -c ',num2str(sol(1)),' -g ',num2str(sol(2)),' -s 3 -p 0.01'];
        ...
        fprintf('iter=%d Params= %4.4f,%4.4f ObjVal=%4.4f\n',iter,GlobalParams,GlobalMin);
        iter=iter+1;
...
BestMSE=GlobalMin;
% BestMSE=GlobalMins(ABCOpts.MaxCycles);

figure;
hold on;
trace = round(trace*10000)/10000;
semilogy(trace(1:iter-1,1),'r*-','LineWidth',1.5);
semilogy(trace(1:iter-1,2),'o-','LineWidth',1.5);
% legend('最佳适应度','平均适应度');
